...
batch_size = 5000 # 128 images trained at a time
...
model.add(Conv2D(32, kernel_size=(3, 3), # 32 filters, kernel = size of filter,
                 ...
                 padding='same')) # input shape for first layer
...
model.add(MaxPooling2D(pool_size=(2, 2)))
#odel.add(MaxPooling2D(pool_size=(3, 3))) # downsamples the input
...
#BatchNormalization(axis=-1)
#model.add(Dense(512, activation='relu')) # batch size = 128 = hidden neuron units
#model.add(Dropout(0.4))
